...
class SumTerm:
    ...
    def to_latex(self) -> str:
        ...
        if self.dummies and isinstance(self.body, Add):
...
class SymExpr:
    ...
    def diff_indexed(self, base: IndexedBase,
        ...
        """
        Symbolic partial derivative wrt ``base[free]`` -- works for *arbitrary*
        nonlinear bodies via ``sp.diff`` (NOT just linear via ``coeff``).

        For each SumTerm and each Indexed occurrence of ``base`` in its body,
        α-rename the dummies that index that occurrence to ``free``, then
        differentiate the renamed body with respect to ``base[free]``.  The
        unconsumed dummies stay as summation indices in the result.
        """
        out: List[SumTerm] = []
    ...
    def substitute_indexed(self, base: IndexedBase,
                           free: Tuple[Symbol, ...],
                           solution: sp.Expr) -> "SymExpr":
        """
        Replace every occurrence of ``base[any_indices]`` with the supplied
        ``solution`` (whose free indices are listed in ``free``), aligning
        ``free`` to the actual indices each occurrence carries.
        """
    ...
    def has_base(self, base: IndexedBase) -> bool:
...
class DualDeriver:
    ...
    def derive_dual(self) -> Dict[str, object]:
        ...
        for vname, vinfo in self.variables.items():
            ...
            target = (vinfo.base[vinfo.free_symbols]
                      if len(vinfo.free_symbols) > 1
                      else vinfo.base[vinfo.free_symbols[0]])
